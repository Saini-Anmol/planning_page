...
# scipy is only needed by the LP solver (Phase 3). Import it lazily so the ETL /
# dataloader / CSV paths work without scipy installed.
try:
...
try:
    from _version import VERSION_LABEL
except ImportError:
    VERSION_LABEL = "v6.0"
...
class Config:
    ...
    # ── press & mould ─────────────────────────────────────────────────────────
    # [Correction C1] A PCR mould has 1 cavity; a press carries 2 moulds, so a
    # press cures 1 x 2 = 2 tyres per cycle. CAVITIES_PER_MOULD was previously
    # mis-set to 2, which (combined with the dropped MOULDS_PER_PRESS factor in
    # the output math) made the production numbers wrong. Real units/cycle is
    # now computed by Config.units_per_cycle().
    CAVITIES_PER_MOULD = 1
    MOULDS_PER_PRESS   = 2
    NEW_MOULD_LIFE     = 3000
    ...
    def units_per_cycle(cls) -> int:
        # Tyres cured in one press stroke = all cavities of all mounted moulds.
        # [Correction C1] LP/rounder/builder previously used CAVITIES_PER_MOULD
        # alone, dropping the MOULDS_PER_PRESS factor and halving press output.
        return cls.CAVITIES_PER_MOULD * cls.MOULDS_PER_PRESS

    @classmethod
    def units_per_cleaning_cycle(cls) -> int:
        return cls.NEW_MOULD_LIFE * cls.units_per_cycle()
...
class ETL:
    ...
    # ── CSV loaders ───────────────────────────────────────────────────────────
    # Consume the canonical CSVs written by dataloader.py. SKUCode is forced to
    # str everywhere so it matches the string keys the MouldTracker stores for
    # mould↔SKU compatibility (an int SKUCode would silently fail every lookup).
    @staticmethod
    def load_demand_from_csv(path: str) -> pd.DataFrame:
        df = pd.read_csv(path, dtype={"SKUCode": str})
        df = df.rename(columns={"Updated_Requirement": "Quantity",
                                "ConsolidatedPriorityScore": "Priority",
                                "Penetration%": "Priority"})
        missing = {"SKUCode", "Quantity", "Priority"} - set(df.columns)
        if missing:
            raise ValueError(f"demand.csv missing columns {missing}; "
                             f"found {list(df.columns)}")
        df["SKUCode"] = df["SKUCode"].str.strip()
        df = (df.groupby("SKUCode", as_index=False)
                .agg(Quantity=("Quantity", "sum"), Priority=("Priority", "max")))
        return df[df["Quantity"] > 0].copy()

    @staticmethod
    def load_cycle_times_from_csv(path: str) -> pd.DataFrame:
        df = pd.read_csv(path, dtype={"SKUCode": str})
        df["SKUCode"] = df["SKUCode"].str.strip()
        return df[["SKUCode", "CycleTime_min"]].drop_duplicates("SKUCode")

    @staticmethod
    def load_machine_allowable_from_csv(path: str) -> pd.DataFrame:
        df = pd.read_csv(path, dtype={"SKUCode": str})
        df["SKUCode"] = df["SKUCode"].str.strip()
        df["Machines"] = df["Machines"].apply(
    ...
    @staticmethod
    def load_gt_inventory_from_csv(path: str) -> pd.DataFrame:
        df = pd.read_csv(path, dtype={"SKUCode": str})
        df["SKUCode"] = df["SKUCode"].str.strip()
        return df[["SKUCode", "GT_Inventory"]]

    @staticmethod
    def load_running_moulds_from_csv(path: str) -> pd.DataFrame:
        df = pd.read_csv(path, dtype={"SKUCode": str, "Machine": str})
        if "MouldLife_remaining" not in df.columns:
            df["MouldLife_remaining"] = Config.NEW_MOULD_LIFE
        df["MouldNos"] = df["MouldNos"].apply(
            lambda x: ast.literal_eval(x) if isinstance(x, str) else x
        )
        if "Num_Moulds" not in df.columns:
            df["Num_Moulds"] = df["MouldNos"].apply(
                lambda v: len(v) if isinstance(v, list) else 1
            )
        return df[["Machine", "SKUCode", "MouldNos", "MouldLife_remaining", "Num_Moulds"]]

    @staticmethod
    def load_mould_master_from_csv(path: str) -> pd.DataFrame:
        # Keep every code as a string (mould/SKU codes must not become ints).
        df = pd.read_csv(path, dtype=str)
        if "Active Flag" in df.columns:
            df = df[df["Active Flag"].astype(str).str.strip().str.lower()
                    .isin(["true", "1", "yes"])]
        return df
...
class LP_Solver:
    ...
    def solve(
        ...
        if linprog is None:
            raise ImportError("scipy is required for the LP solve. "
                              "`pip install scipy`")
...
class Rounder:
    ...
    def _row(self, mach, a_or_row, cycles, ct, priority):
        ...
        return {
            ...
            "CycleTime_min": ct,
            "Cycles":        cycles,
            "Units_Planned": cycles * Config.units_per_cycle(),  # [C1]
        ...
        for row in sorted_rows:
            ...
            for mach in eligible:
                ...
                extra_c = min(int(available / ct),
                              math.ceil(needed / Config.units_per_cycle()))  # [C1]
                if extra_c <= 0:
                    continue
                machine_cap[mach] -= extra_c * ct + co
                units = extra_c * Config.units_per_cycle()  # [C1]
                planned[sku] += units
                needed       -= units
...
class ScheduleBuilder:
    ...
    def _split_block(
        ...
        total_units = int(total_mins / ct) * Config.units_per_cycle()  # [C1]
        produced    = 0
        curr        = start
        ...
        while curr < end and produced < total_units:
            ...
            mins_this_run  = math.ceil(units_this_run / Config.units_per_cycle()) * ct  # [C1]
            run_end        = min(curr + timedelta(minutes=mins_this_run), end)

            inner = curr
            run_produced = 0
            run_total = int((run_end - curr).total_seconds() / 60 / ct) * Config.units_per_cycle()  # [C1]
            ...
            while inner < run_end:
                ...
                if slice_end == run_end:
                    ...
                    qty = int(dur / ct) * Config.units_per_cycle()  # [C1]
...
class JK_LP_Curing_Scheduler_v2:   # name kept for backwards compatibility
    ...
    def _prepare_skus(self, df_demand, df_cycles, df_allow, df_gt,
        ...
        for _, r in df_demand.iterrows():
            ...
            gt       = int(gt_map.get(sku, 0))
            dm       = math.ceil(qty / Config.units_per_cycle()) * ct if ct else 0  # [C1]
        ...
        for sku, remainder in demand_remainder.items():
            ...
            if remainder == 0:
                ...
                df_lp.loc[mask, "Demand_Mins"] = (
                    math.ceil(remainder / Config.units_per_cycle())  # [C1]
...
class ExcelExporter:
    ...
    @staticmethod
    def _daily_table(df_shift: pd.DataFrame) -> pd.DataFrame:
        """Total tyres cured per day (excludes changeover & cleaning rows),
        broken down by shift, with a running cumulative total."""
        cols = ["Date", "Shift_A", "Shift_B", "Shift_C",
        ...
        piv = (prod.pivot_table(index="Date", columns="Shift", values="Qty",
                   ...
                   .reset_index())
        for sh in ["A", "B", "C"]:
            if sh not in piv.columns:
                piv[sh] = 0
        piv = piv.sort_values("Date").reset_index(drop=True)
        out = pd.DataFrame({
            "Date":    piv["Date"],
            "Shift_A": piv["A"].astype(int),
            "Shift_B": piv["B"].astype(int),
            "Shift_C": piv["C"].astype(int),
        })
        out["Total_Units"]      = out[["Shift_A", "Shift_B", "Shift_C"]].sum(axis=1)
        out["Cumulative_Units"] = out["Total_Units"].cumsum()
        return out[cols]
    ...
    def export(self, results: dict):
        ...
        with pd.ExcelWriter(self.path, engine="openpyxl") as writer:
            ...
            self._title(ws,f"PCR CURING {VERSION_LABEL} — DEMAND FULFILLMENT",kpi,len(cols1))
            ...
            self._title(ws2,f"MACHINE-WISE SCHEDULE — PCR {VERSION_LABEL}",kpi,len(cols2))
            ...
            self._title(ws3,f"SHIFT-WISE SCHEDULE — PCR {VERSION_LABEL}",kpi,len(cols3))
            ...
            self._title(ws4,f"PRESS UTILIZATION — PCR {VERSION_LABEL}",
            ...
            # ── Daily Curing ───────────────────────────────────────────────────
            df_daily = self._daily_table(df_shift)
            cols6 = list(df_daily.columns)
            df_daily.to_excel(writer, sheet_name="Daily Curing", index=False)
            ws6 = writer.book["Daily Curing"]
            tot_cured = int(df_daily["Total_Units"].sum()) if not df_daily.empty else 0
            ndays     = len(df_daily)
            avg_day   = int(round(tot_cured / ndays)) if ndays else 0
            peak      = int(df_daily["Total_Units"].max()) if ndays else 0
            self._title(ws6, "DAILY CURING — UNITS CURED PER DAY",
                        f"Total cured: {tot_cured:,} | Days: {ndays} | "
                        f"Avg/day: {avg_day:,} | Peak/day: {peak:,}",
                        len(cols6))
            self._hdr_row(ws6, 3, len(cols6))
            for ci, w in enumerate([14, 12, 12, 12, 14, 16], 1):
                ws6.column_dimensions[get_column_letter(ci)].width = w
            for ri in range(4, ws6.max_row + 1):
                bf = "grey" if ri % 2 == 0 else "white"
                for ci in range(1, len(cols6) + 1):
            ...
            if not df_daily.empty:
                tr = ws6.max_row + 1
                for ci in range(1, len(cols6) + 1):
                    c = ws6.cell(tr, ci); c.fill = self.F("navy"); c.font = self._hf()
                    c.border = self._b()
                    c.alignment = Alignment(horizontal="center", vertical="center")
                ws6.cell(tr, 1).value = "TOTAL"
                for ci, col in [(2, "Shift_A"), (3, "Shift_B"),
                                (4, "Shift_C"), (5, "Total_Units")]:
                    ws6.cell(tr, ci).value = int(df_daily[col].sum())
                    ws6.cell(tr, ci).number_format = "#,##0"
                ws6.cell(tr, 6).value = tot_cured
                ws6.cell(tr, 6).number_format = "#,##0"
...
def run_from_csv(
    ...
    """
    Run the scheduler from the canonical CSV folder produced by dataloader.py.

    Expected files in `input_dir`:
        demand.csv            (you add this — SKUCode, Quantity, Priority)
        cycle_times.csv       SKUCode, CycleTime_min
        machine_allowable.csv SKUCode, Machines (list literal, e.g. "[4401, 4402]")
        gt_inventory.csv      SKUCode, GT_Inventory
        mould_master.csv      mould master (MouldNo / Matl.Code / Active Flag ...)
        running_moulds.csv    Machine, SKUCode, MouldNos (list), MouldLife_remaining,
                              Num_Moulds                     (optional)
    """
    def p(fname: str) -> str:
    ...
    if missing:
        raise FileNotFoundError(
            ...
            f"Run dataloader.py to generate the masters, then add demand.csv."
        )
...
if __name__ == "__main__":
    results = run_from_database(
        demand_csv = r"C:\Users\Pranjay\Downloads\Book4(Sheet4).csv",
